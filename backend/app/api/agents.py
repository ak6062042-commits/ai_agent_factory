...
def create_agent(
    ...
    # TODO: add a log here later 
    if len(documents) > MAX_UPLOADS:
        raise HTTPException(
            status_code=400,
            detail=f"Too many files: max {MAX_UPLOADS} allowed, got {len(documents)}",
        )
    return agent_services.create_agent(db, tenant, agent_name, website_url, documents)
